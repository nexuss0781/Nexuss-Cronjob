...
export function MonitorCard({ monitor, onDelete }: MonitorCardProps) {
  ...
  const uptimePercent = monitor.totalChecks > 0
    ? Math.round(((monitor.totalChecks - monitor.failedChecks) / monitor.totalChecks) * 100)
    : 0;

  return (
    <div className="rounded-2xl border border-white/5 bg-white/[0.02] hover:bg-white/[0.04] transition-colors">
      <div className="p-5">
        <div className="flex items-start justify-between mb-3">
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-1.5 text-xs text-zinc-500">
              <span className="px-1.5 py-0.5 rounded bg-white/5 font-mono text-zinc-400">
                {monitor.method}
              </span>
              <a
                href={monitor.url}
                ...
                className="flex items-center gap-1 hover:text-zinc-300 truncate max-w-[300px]"
              >
                {monitor.url}
        ...
        <div className="grid grid-cols-4 gap-3 mt-4">
          ...
          <div>
            <div className="text-xs text-zinc-500 mb-0.5">Status Code</div>
            <div className="text-sm font-medium">
              {monitor.lastStatusCode != null ? monitor.lastStatusCode : '—'}
            </div>
          </div>
          <div>
            <div className="text-xs text-zinc-500 mb-0.5">Interval</div>
            ...
            <div className="space-y-2 max-h-60 overflow-y-auto">
              {history.map((check) => (
                ...
                >
                  ...
                  <span className="text-zinc-400">{check.responseTime}ms</span>
                  {check.error && (
