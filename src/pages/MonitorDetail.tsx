  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 space-y-4 sm:space-y-6">
      <div className="flex items-start gap-3 sm:gap-4">
        <button
          onClick={() => navigate('/')}
          className="p-2 rounded-lg hover:bg-white/5 text-zinc-400 hover:text-white transition-colors cursor-pointer shrink-0 mt-1"
        >
          <ArrowLeft className="w-5 h-5" />
        </button>
        <div className="flex-1 min-w-0">
          <div className="flex flex-wrap items-center gap-2 sm:gap-3">
            <h1 className="text-xl sm:text-2xl font-bold tracking-tight">{monitor.name}</h1>
            <StatusBadge status={monitor.status} />
          </div>
          <div className="flex items-center gap-2 mt-1 text-xs sm:text-sm text-zinc-500">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        <div className="rounded-2xl border border-white/5 bg-white/[0.02] p-4 sm:p-5">
          <div className="text-[10px] sm:text-xs text-zinc-500 mb-1">Uptime</div>
          <div className="text-xl sm:text-2xl font-bold">{uptimePercent}%</div>
        </div>
        <div className="rounded-2xl border border-white/5 bg-white/[0.02] p-4 sm:p-5">
          <div className="text-[10px] sm:text-xs text-zinc-500 mb-1">Avg Response</div>
          <div className="text-xl sm:text-2xl font-bold">
            {monitor.lastResponseTime != null ? `${monitor.lastResponseTime}ms` : '—'}
          </div>
        </div>
        <div className="rounded-2xl border border-white/5 bg-white/[0.02] p-4 sm:p-5">
          <div className="text-[10px] sm:text-xs text-zinc-500 mb-1">Total Checks</div>
          <div className="text-xl sm:text-2xl font-bold">{monitor.totalChecks}</div>
        </div>
        <div className="rounded-2xl border border-white/5 bg-white/[0.02] p-4 sm:p-5">
          <div className="text-[10px] sm:text-xs text-zinc-500 mb-1">Interval</div>
          <div className="text-xl sm:text-2xl font-bold flex items-center gap-1.5">
            <Timer className="w-4 h-4 sm:w-5 sm:h-5 text-zinc-500" />
            {monitor.interval}s
          </div>
        </div>
      </div>
        ...
        {history.length === 0 ? (
          ...
          <div className="space-y-2 max-h-[400px] overflow-y-auto">
            {history.map((c) => (
              <div
                key={c.id}
                className="flex items-center gap-2 sm:gap-3 text-sm py-2 border-b border-white/5 last:border-0"
              >
                {c.status === 'up' ? (
                ...
                <span className="text-zinc-400 text-xs sm:text-sm shrink-0">{c.responseTime}ms</span>
                {c.error && (
                  <span className="text-red-400 text-xs truncate flex-1">{c.error}</span>
                )}
